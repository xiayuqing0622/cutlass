...
using         LayoutD     = LayoutC;              // Layout type  D matrix operands
constexpr int AlignmentD  = AlignmentC;    // Memory access granularity/alignment of D matrix in units of elements (up to 16 bytes)
...
using TileShapeMain       = Shape<_128,_128,_64>;                           // Threadblock-level tile size
using ClusterShape        = Shape<_2,_1,_1>;                                // Shape of the threadblocks in a cluster
using EpilogueTileType    = cutlass::epilogue::collective::EpilogueTileAuto; //default
// using StageCountType = cutlass::gemm::collective::StageCountAuto;           // Stage count maximized based on the tile size
using KernelScheduleType =  cutlass::gemm::KernelTmaWarpSpecializedPingpong;// default: cutlass::gemm::collective::KernelScheduleAuto;       // Kernel to launch based on the default setting in the Collective Builder
using EpilogueScheduleType = cutlass::epilogue::TmaWarpSpecialized;  // default: cutlass::epilogue::EpilogueScheduleAuto;
using FusionOpOrCallbacks = cutlass::epilogue::fusion::LinearCombination<ElementD,ElementCompute,ElementC,ElementCompute>;
...
// default: using StageCountType = cutlass::gemm::collective::StageCountAuto;           // Stage count maximized based on the tile size
...
typename Gemm::Arguments args_from_options(const Options &options)
{
  ...
  arguments.scheduler.raster_order =  RasterOrderOptions::Heuristic; // choose from[RasterOrderOptions::AlongN, RasterOrderOptions::AlongM, RasterOrderOptions::Heuristic]   
