...
using         ElementA    = double;                                   // Element type for A matrix operand
using         LayoutA     =  cutlass::layout::RowMajor;                                    // Layout type for A matrix operand
constexpr int AlignmentA  =  1;//128 / cutlass::sizeof_bits<ElementA>::value;    // Memory access granularity/alignment of A matrix in units of elements (up to 16 bytes)

// B matrix configuration
using         ElementB    = double;                                   // Element type for B matrix operand
...
using         ElementC    = double;                            // Element type for C matrix operands
using         LayoutC     =  cutlass::layout::RowMajor;                       // Layout type for C matrix operands

// Core kernel configurations
using ElementAccumulator  = double;                                          // Element type for internal accumulation
...
using FusionOpOrCallbacks = cutlass::epilogue::thread::LinearCombination<ElementC,1,ElementAccumulator,ElementEpilogue>;
using SwizzlingFunctor = cutlass::gemm::threadblock::GemmIdentityThreadblockSwizzle<8>;
constexpr int NumStages = 3;
using MathOperation = cutlass::arch::OpMultiplyAdd;

using GemmKernel = cutlass::gemm::kernel::DefaultGemmUniversal<
    ElementB, LayoutB, cutlass::ComplexTransform::kNone, AlignmentB,    // transposed B operand
    ElementA, LayoutA, cutlass::ComplexTransform::kNone, AlignmentA,    // transposed A operand
    ElementC, LayoutC,
    ElementAccumulator,
    OperatorClass,
    ArchTag,
